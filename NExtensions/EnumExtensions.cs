...
namespace NExtensions
{
    ...
    public static class Enums
    {
        ...
        public static IEnumerable<T> GetValues<T>(this Type type) where T : Enum
        {
            return GetValues<T>();
        }
        ...
        public static T ToEnum<T>(this string input) where T : Enum
        {
            var values = GetValues<T>()
                .Where(x => x.ToString().Equals(input, StringComparison.InvariantCultureIgnoreCase)).ToList();
            ...
            var descriptions = GetValues<T>()
                .Where(x => x.GetDescription().Equals(input, StringComparison.InvariantCultureIgnoreCase)).ToList();
            ...
            throw new ArgumentException(
                "Cannot find enum value '{0}' for type {1}".FormatWith(input ?? "<NULL>", typeof(T).Name));
        }
        ...
        public static T ToEnum<T>(this long intValue) where T : Enum
        {
            if (!typeof(T).IsEnum)
            {
                throw new Exception("T must be an Enumeration type.");
